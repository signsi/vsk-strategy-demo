...
<head>
    ...
    <style type="text/css">
        body { margin-left: 2%; margin-right: 2%; font:normal 90% arial,helvetica,sanserif; color:#000000; }
        table.sortable tr th { font-weight: bold; text-align:left; background:#a6caf0; }
        table.sortable tr td { background:#eeeee0; vertical-align: top; }
        ...
        div.small{ font:normal 60% arial, helvetica, sanserif; font-type: italic; }
...
<body>
    ...
    <xsl:for-each select="file">
	    ...
	    <xsl:for-each select="violation">
			...
			<td style="padding: 3px" align="left" width="100%"><xsl:if test="@externalInfoUrl">
               <a><xsl:attribute name="href"><xsl:value-of select="@externalInfoUrl"/></xsl:attribute><xsl:call-template name="message"/></a></xsl:if><xsl:if test="not(@externalInfoUrl)"><xsl:call-template name="message"/></xsl:if>
               <div class="small"><xsl:value-of disable-output-escaping="yes" select="@rule"/></div>
            </td>
		    </tr>
	    </xsl:for-each>
		</table>
